/*
AskNews API

AskNews API [![status](https://status.asknews.app/api/badge/2/status?style=for-the-badge)](https://status.asknews.app/status/prod)

API version: 0.31.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package asknews

import (
	"encoding/json"
	"time"
	"fmt"
)

// checks if the SearchResponseDictItem1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SearchResponseDictItem1{}

// SearchResponseDictItem1 struct for SearchResponseDictItem1
type SearchResponseDictItem1 struct {
	ArticleUrl string `json:"article_url"`
	ArticleId string `json:"article_id"`
	Classification Classification `json:"classification"`
	Country string `json:"country"`
	SourceId string `json:"source_id"`
	PageRank int32 `json:"page_rank"`
	DomainUrl string `json:"domain_url"`
	EngTitle string `json:"eng_title"`
	Entities Entities1 `json:"entities"`
	ImageUrl NullableString `json:"image_url,omitempty"`
	Keywords []string `json:"keywords"`
	Language string `json:"language"`
	PubDate time.Time `json:"pub_date"`
	Summary string `json:"summary"`
	KeyPoints []string `json:"key_points,omitempty"`
	Title string `json:"title"`
	Sentiment int32 `json:"sentiment"`
	CentroidDistance float32 `json:"centroid_distance"`
	ClusterProbability float32 `json:"cluster_probability"`
	MarkdownCitation *string `json:"markdown_citation,omitempty"`
	Provocative *string `json:"provocative,omitempty"`
	ReportingVoice *ReportingVoice1 `json:"reporting_voice,omitempty"`
	EntityRelationGraph NullableGraphRelationships `json:"entity_relation_graph,omitempty"`
	GeoCoordinates map[string]GeoCoordinate1 `json:"geo_coordinates,omitempty"`
	Continent NullableString `json:"continent,omitempty"`
	Assets NullableAssets1 `json:"assets,omitempty"`
	SocialEmbeds interface{} `json:"social_embeds,omitempty"`
	Bias NullableString `json:"bias,omitempty"`
	Authors []Author1 `json:"authors,omitempty"`
	FullText NullableString `json:"full_text,omitempty"`
	OriginalLanguageSummary NullableString `json:"original_language_summary,omitempty"`
	ImageDescription NullableString `json:"image_description,omitempty"`
	CrawlDate NullableTime `json:"crawl_date,omitempty"`
	ContentType NullableString `json:"content_type,omitempty"`
	AsStringKey string `json:"as_string_key"`
	AdditionalProperties map[string]interface{}
}

type _SearchResponseDictItem1 SearchResponseDictItem1

// NewSearchResponseDictItem1 instantiates a new SearchResponseDictItem1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSearchResponseDictItem1(articleUrl string, articleId string, classification Classification, country string, sourceId string, pageRank int32, domainUrl string, engTitle string, entities Entities1, keywords []string, language string, pubDate time.Time, summary string, title string, sentiment int32, centroidDistance float32, clusterProbability float32, asStringKey string) *SearchResponseDictItem1 {
	this := SearchResponseDictItem1{}
	this.ArticleUrl = articleUrl
	this.ArticleId = articleId
	this.Classification = classification
	this.Country = country
	this.SourceId = sourceId
	this.PageRank = pageRank
	this.DomainUrl = domainUrl
	this.EngTitle = engTitle
	this.Entities = entities
	this.Keywords = keywords
	this.Language = language
	this.PubDate = pubDate
	this.Summary = summary
	this.Title = title
	this.Sentiment = sentiment
	this.CentroidDistance = centroidDistance
	this.ClusterProbability = clusterProbability
	var markdownCitation string = ""
	this.MarkdownCitation = &markdownCitation
	var provocative string = "unknown"
	this.Provocative = &provocative
	var reportingVoice ReportingVoice1
	this.ReportingVoice = &reportingVoice
	this.AsStringKey = asStringKey
	return &this
}

// NewSearchResponseDictItem1WithDefaults instantiates a new SearchResponseDictItem1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSearchResponseDictItem1WithDefaults() *SearchResponseDictItem1 {
	this := SearchResponseDictItem1{}
	var markdownCitation string = ""
	this.MarkdownCitation = &markdownCitation
	var provocative string = "unknown"
	this.Provocative = &provocative
	var reportingVoice ReportingVoice1
	this.ReportingVoice = &reportingVoice
	return &this
}

// GetArticleUrl returns the ArticleUrl field value
func (o *SearchResponseDictItem1) GetArticleUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ArticleUrl
}

// GetArticleUrlOk returns a tuple with the ArticleUrl field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetArticleUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ArticleUrl, true
}

// SetArticleUrl sets field value
func (o *SearchResponseDictItem1) SetArticleUrl(v string) {
	o.ArticleUrl = v
}

// GetArticleId returns the ArticleId field value
func (o *SearchResponseDictItem1) GetArticleId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ArticleId
}

// GetArticleIdOk returns a tuple with the ArticleId field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetArticleIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ArticleId, true
}

// SetArticleId sets field value
func (o *SearchResponseDictItem1) SetArticleId(v string) {
	o.ArticleId = v
}

// GetClassification returns the Classification field value
func (o *SearchResponseDictItem1) GetClassification() Classification {
	if o == nil {
		var ret Classification
		return ret
	}

	return o.Classification
}

// GetClassificationOk returns a tuple with the Classification field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetClassificationOk() (*Classification, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Classification, true
}

// SetClassification sets field value
func (o *SearchResponseDictItem1) SetClassification(v Classification) {
	o.Classification = v
}

// GetCountry returns the Country field value
func (o *SearchResponseDictItem1) GetCountry() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Country
}

// GetCountryOk returns a tuple with the Country field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetCountryOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Country, true
}

// SetCountry sets field value
func (o *SearchResponseDictItem1) SetCountry(v string) {
	o.Country = v
}

// GetSourceId returns the SourceId field value
func (o *SearchResponseDictItem1) GetSourceId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SourceId
}

// GetSourceIdOk returns a tuple with the SourceId field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetSourceIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SourceId, true
}

// SetSourceId sets field value
func (o *SearchResponseDictItem1) SetSourceId(v string) {
	o.SourceId = v
}

// GetPageRank returns the PageRank field value
func (o *SearchResponseDictItem1) GetPageRank() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.PageRank
}

// GetPageRankOk returns a tuple with the PageRank field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetPageRankOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PageRank, true
}

// SetPageRank sets field value
func (o *SearchResponseDictItem1) SetPageRank(v int32) {
	o.PageRank = v
}

// GetDomainUrl returns the DomainUrl field value
func (o *SearchResponseDictItem1) GetDomainUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.DomainUrl
}

// GetDomainUrlOk returns a tuple with the DomainUrl field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetDomainUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DomainUrl, true
}

// SetDomainUrl sets field value
func (o *SearchResponseDictItem1) SetDomainUrl(v string) {
	o.DomainUrl = v
}

// GetEngTitle returns the EngTitle field value
func (o *SearchResponseDictItem1) GetEngTitle() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.EngTitle
}

// GetEngTitleOk returns a tuple with the EngTitle field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetEngTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.EngTitle, true
}

// SetEngTitle sets field value
func (o *SearchResponseDictItem1) SetEngTitle(v string) {
	o.EngTitle = v
}

// GetEntities returns the Entities field value
func (o *SearchResponseDictItem1) GetEntities() Entities1 {
	if o == nil {
		var ret Entities1
		return ret
	}

	return o.Entities
}

// GetEntitiesOk returns a tuple with the Entities field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetEntitiesOk() (*Entities1, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Entities, true
}

// SetEntities sets field value
func (o *SearchResponseDictItem1) SetEntities(v Entities1) {
	o.Entities = v
}

// GetImageUrl returns the ImageUrl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetImageUrl() string {
	if o == nil || IsNil(o.ImageUrl.Get()) {
		var ret string
		return ret
	}
	return *o.ImageUrl.Get()
}

// GetImageUrlOk returns a tuple with the ImageUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetImageUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ImageUrl.Get(), o.ImageUrl.IsSet()
}

// HasImageUrl returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasImageUrl() bool {
	if o != nil && o.ImageUrl.IsSet() {
		return true
	}

	return false
}

// SetImageUrl gets a reference to the given NullableString and assigns it to the ImageUrl field.
func (o *SearchResponseDictItem1) SetImageUrl(v string) {
	o.ImageUrl.Set(&v)
}
// SetImageUrlNil sets the value for ImageUrl to be an explicit nil
func (o *SearchResponseDictItem1) SetImageUrlNil() {
	o.ImageUrl.Set(nil)
}

// UnsetImageUrl ensures that no value is present for ImageUrl, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetImageUrl() {
	o.ImageUrl.Unset()
}

// GetKeywords returns the Keywords field value
func (o *SearchResponseDictItem1) GetKeywords() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.Keywords
}

// GetKeywordsOk returns a tuple with the Keywords field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetKeywordsOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Keywords, true
}

// SetKeywords sets field value
func (o *SearchResponseDictItem1) SetKeywords(v []string) {
	o.Keywords = v
}

// GetLanguage returns the Language field value
func (o *SearchResponseDictItem1) GetLanguage() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Language
}

// GetLanguageOk returns a tuple with the Language field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetLanguageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Language, true
}

// SetLanguage sets field value
func (o *SearchResponseDictItem1) SetLanguage(v string) {
	o.Language = v
}

// GetPubDate returns the PubDate field value
func (o *SearchResponseDictItem1) GetPubDate() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.PubDate
}

// GetPubDateOk returns a tuple with the PubDate field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetPubDateOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PubDate, true
}

// SetPubDate sets field value
func (o *SearchResponseDictItem1) SetPubDate(v time.Time) {
	o.PubDate = v
}

// GetSummary returns the Summary field value
func (o *SearchResponseDictItem1) GetSummary() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Summary
}

// GetSummaryOk returns a tuple with the Summary field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetSummaryOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Summary, true
}

// SetSummary sets field value
func (o *SearchResponseDictItem1) SetSummary(v string) {
	o.Summary = v
}

// GetKeyPoints returns the KeyPoints field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetKeyPoints() []string {
	if o == nil {
		var ret []string
		return ret
	}
	return o.KeyPoints
}

// GetKeyPointsOk returns a tuple with the KeyPoints field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetKeyPointsOk() ([]string, bool) {
	if o == nil || IsNil(o.KeyPoints) {
		return nil, false
	}
	return o.KeyPoints, true
}

// HasKeyPoints returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasKeyPoints() bool {
	if o != nil && !IsNil(o.KeyPoints) {
		return true
	}

	return false
}

// SetKeyPoints gets a reference to the given []string and assigns it to the KeyPoints field.
func (o *SearchResponseDictItem1) SetKeyPoints(v []string) {
	o.KeyPoints = v
}

// GetTitle returns the Title field value
func (o *SearchResponseDictItem1) GetTitle() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Title
}

// GetTitleOk returns a tuple with the Title field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Title, true
}

// SetTitle sets field value
func (o *SearchResponseDictItem1) SetTitle(v string) {
	o.Title = v
}

// GetSentiment returns the Sentiment field value
func (o *SearchResponseDictItem1) GetSentiment() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Sentiment
}

// GetSentimentOk returns a tuple with the Sentiment field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetSentimentOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Sentiment, true
}

// SetSentiment sets field value
func (o *SearchResponseDictItem1) SetSentiment(v int32) {
	o.Sentiment = v
}

// GetCentroidDistance returns the CentroidDistance field value
func (o *SearchResponseDictItem1) GetCentroidDistance() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.CentroidDistance
}

// GetCentroidDistanceOk returns a tuple with the CentroidDistance field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetCentroidDistanceOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CentroidDistance, true
}

// SetCentroidDistance sets field value
func (o *SearchResponseDictItem1) SetCentroidDistance(v float32) {
	o.CentroidDistance = v
}

// GetClusterProbability returns the ClusterProbability field value
func (o *SearchResponseDictItem1) GetClusterProbability() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.ClusterProbability
}

// GetClusterProbabilityOk returns a tuple with the ClusterProbability field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetClusterProbabilityOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ClusterProbability, true
}

// SetClusterProbability sets field value
func (o *SearchResponseDictItem1) SetClusterProbability(v float32) {
	o.ClusterProbability = v
}

// GetMarkdownCitation returns the MarkdownCitation field value if set, zero value otherwise.
func (o *SearchResponseDictItem1) GetMarkdownCitation() string {
	if o == nil || IsNil(o.MarkdownCitation) {
		var ret string
		return ret
	}
	return *o.MarkdownCitation
}

// GetMarkdownCitationOk returns a tuple with the MarkdownCitation field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetMarkdownCitationOk() (*string, bool) {
	if o == nil || IsNil(o.MarkdownCitation) {
		return nil, false
	}
	return o.MarkdownCitation, true
}

// HasMarkdownCitation returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasMarkdownCitation() bool {
	if o != nil && !IsNil(o.MarkdownCitation) {
		return true
	}

	return false
}

// SetMarkdownCitation gets a reference to the given string and assigns it to the MarkdownCitation field.
func (o *SearchResponseDictItem1) SetMarkdownCitation(v string) {
	o.MarkdownCitation = &v
}

// GetProvocative returns the Provocative field value if set, zero value otherwise.
func (o *SearchResponseDictItem1) GetProvocative() string {
	if o == nil || IsNil(o.Provocative) {
		var ret string
		return ret
	}
	return *o.Provocative
}

// GetProvocativeOk returns a tuple with the Provocative field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetProvocativeOk() (*string, bool) {
	if o == nil || IsNil(o.Provocative) {
		return nil, false
	}
	return o.Provocative, true
}

// HasProvocative returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasProvocative() bool {
	if o != nil && !IsNil(o.Provocative) {
		return true
	}

	return false
}

// SetProvocative gets a reference to the given string and assigns it to the Provocative field.
func (o *SearchResponseDictItem1) SetProvocative(v string) {
	o.Provocative = &v
}

// GetReportingVoice returns the ReportingVoice field value if set, zero value otherwise.
func (o *SearchResponseDictItem1) GetReportingVoice() ReportingVoice1 {
	if o == nil || IsNil(o.ReportingVoice) {
		var ret ReportingVoice1
		return ret
	}
	return *o.ReportingVoice
}

// GetReportingVoiceOk returns a tuple with the ReportingVoice field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetReportingVoiceOk() (*ReportingVoice1, bool) {
	if o == nil || IsNil(o.ReportingVoice) {
		return nil, false
	}
	return o.ReportingVoice, true
}

// HasReportingVoice returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasReportingVoice() bool {
	if o != nil && !IsNil(o.ReportingVoice) {
		return true
	}

	return false
}

// SetReportingVoice gets a reference to the given ReportingVoice1 and assigns it to the ReportingVoice field.
func (o *SearchResponseDictItem1) SetReportingVoice(v ReportingVoice1) {
	o.ReportingVoice = &v
}

// GetEntityRelationGraph returns the EntityRelationGraph field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetEntityRelationGraph() GraphRelationships {
	if o == nil || IsNil(o.EntityRelationGraph.Get()) {
		var ret GraphRelationships
		return ret
	}
	return *o.EntityRelationGraph.Get()
}

// GetEntityRelationGraphOk returns a tuple with the EntityRelationGraph field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetEntityRelationGraphOk() (*GraphRelationships, bool) {
	if o == nil {
		return nil, false
	}
	return o.EntityRelationGraph.Get(), o.EntityRelationGraph.IsSet()
}

// HasEntityRelationGraph returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasEntityRelationGraph() bool {
	if o != nil && o.EntityRelationGraph.IsSet() {
		return true
	}

	return false
}

// SetEntityRelationGraph gets a reference to the given NullableGraphRelationships and assigns it to the EntityRelationGraph field.
func (o *SearchResponseDictItem1) SetEntityRelationGraph(v GraphRelationships) {
	o.EntityRelationGraph.Set(&v)
}
// SetEntityRelationGraphNil sets the value for EntityRelationGraph to be an explicit nil
func (o *SearchResponseDictItem1) SetEntityRelationGraphNil() {
	o.EntityRelationGraph.Set(nil)
}

// UnsetEntityRelationGraph ensures that no value is present for EntityRelationGraph, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetEntityRelationGraph() {
	o.EntityRelationGraph.Unset()
}

// GetGeoCoordinates returns the GeoCoordinates field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetGeoCoordinates() map[string]GeoCoordinate1 {
	if o == nil {
		var ret map[string]GeoCoordinate1
		return ret
	}
	return o.GeoCoordinates
}

// GetGeoCoordinatesOk returns a tuple with the GeoCoordinates field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetGeoCoordinatesOk() (map[string]GeoCoordinate1, bool) {
	if o == nil || IsNil(o.GeoCoordinates) {
		return map[string]GeoCoordinate1{}, false
	}
	return o.GeoCoordinates, true
}

// HasGeoCoordinates returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasGeoCoordinates() bool {
	if o != nil && !IsNil(o.GeoCoordinates) {
		return true
	}

	return false
}

// SetGeoCoordinates gets a reference to the given map[string]GeoCoordinate1 and assigns it to the GeoCoordinates field.
func (o *SearchResponseDictItem1) SetGeoCoordinates(v map[string]GeoCoordinate1) {
	o.GeoCoordinates = v
}

// GetContinent returns the Continent field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetContinent() string {
	if o == nil || IsNil(o.Continent.Get()) {
		var ret string
		return ret
	}
	return *o.Continent.Get()
}

// GetContinentOk returns a tuple with the Continent field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetContinentOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Continent.Get(), o.Continent.IsSet()
}

// HasContinent returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasContinent() bool {
	if o != nil && o.Continent.IsSet() {
		return true
	}

	return false
}

// SetContinent gets a reference to the given NullableString and assigns it to the Continent field.
func (o *SearchResponseDictItem1) SetContinent(v string) {
	o.Continent.Set(&v)
}
// SetContinentNil sets the value for Continent to be an explicit nil
func (o *SearchResponseDictItem1) SetContinentNil() {
	o.Continent.Set(nil)
}

// UnsetContinent ensures that no value is present for Continent, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetContinent() {
	o.Continent.Unset()
}

// GetAssets returns the Assets field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetAssets() Assets1 {
	if o == nil || IsNil(o.Assets.Get()) {
		var ret Assets1
		return ret
	}
	return *o.Assets.Get()
}

// GetAssetsOk returns a tuple with the Assets field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetAssetsOk() (*Assets1, bool) {
	if o == nil {
		return nil, false
	}
	return o.Assets.Get(), o.Assets.IsSet()
}

// HasAssets returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasAssets() bool {
	if o != nil && o.Assets.IsSet() {
		return true
	}

	return false
}

// SetAssets gets a reference to the given NullableAssets1 and assigns it to the Assets field.
func (o *SearchResponseDictItem1) SetAssets(v Assets1) {
	o.Assets.Set(&v)
}
// SetAssetsNil sets the value for Assets to be an explicit nil
func (o *SearchResponseDictItem1) SetAssetsNil() {
	o.Assets.Set(nil)
}

// UnsetAssets ensures that no value is present for Assets, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetAssets() {
	o.Assets.Unset()
}

// GetSocialEmbeds returns the SocialEmbeds field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetSocialEmbeds() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.SocialEmbeds
}

// GetSocialEmbedsOk returns a tuple with the SocialEmbeds field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetSocialEmbedsOk() (interface{}, bool) {
	if o == nil || IsNil(o.SocialEmbeds) {
		return nil, false
	}
	return o.SocialEmbeds, true
}

// HasSocialEmbeds returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasSocialEmbeds() bool {
	if o != nil && !IsNil(o.SocialEmbeds) {
		return true
	}

	return false
}

// SetSocialEmbeds gets a reference to the given []string and assigns it to the SocialEmbeds field.
func (o *SearchResponseDictItem1) SetSocialEmbeds(v []string) {
	o.SocialEmbeds = v
}

// GetBias returns the Bias field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetBias() string {
	if o == nil || IsNil(o.Bias.Get()) {
		var ret string
		return ret
	}
	return *o.Bias.Get()
}

// GetBiasOk returns a tuple with the Bias field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetBiasOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Bias.Get(), o.Bias.IsSet()
}

// HasBias returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasBias() bool {
	if o != nil && o.Bias.IsSet() {
		return true
	}

	return false
}

// SetBias gets a reference to the given NullableString and assigns it to the Bias field.
func (o *SearchResponseDictItem1) SetBias(v string) {
	o.Bias.Set(&v)
}
// SetBiasNil sets the value for Bias to be an explicit nil
func (o *SearchResponseDictItem1) SetBiasNil() {
	o.Bias.Set(nil)
}

// UnsetBias ensures that no value is present for Bias, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetBias() {
	o.Bias.Unset()
}

// GetAuthors returns the Authors field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetAuthors() []Author1 {
	if o == nil {
		var ret []Author1
		return ret
	}
	return o.Authors
}

// GetAuthorsOk returns a tuple with the Authors field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetAuthorsOk() ([]Author1, bool) {
	if o == nil || IsNil(o.Authors) {
		return nil, false
	}
	return o.Authors, true
}

// HasAuthors returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasAuthors() bool {
	if o != nil && !IsNil(o.Authors) {
		return true
	}

	return false
}

// SetAuthors gets a reference to the given []Author1 and assigns it to the Authors field.
func (o *SearchResponseDictItem1) SetAuthors(v []Author1) {
	o.Authors = v
}

// GetFullText returns the FullText field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetFullText() string {
	if o == nil || IsNil(o.FullText.Get()) {
		var ret string
		return ret
	}
	return *o.FullText.Get()
}

// GetFullTextOk returns a tuple with the FullText field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetFullTextOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FullText.Get(), o.FullText.IsSet()
}

// HasFullText returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasFullText() bool {
	if o != nil && o.FullText.IsSet() {
		return true
	}

	return false
}

// SetFullText gets a reference to the given NullableString and assigns it to the FullText field.
func (o *SearchResponseDictItem1) SetFullText(v string) {
	o.FullText.Set(&v)
}
// SetFullTextNil sets the value for FullText to be an explicit nil
func (o *SearchResponseDictItem1) SetFullTextNil() {
	o.FullText.Set(nil)
}

// UnsetFullText ensures that no value is present for FullText, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetFullText() {
	o.FullText.Unset()
}

// GetOriginalLanguageSummary returns the OriginalLanguageSummary field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetOriginalLanguageSummary() string {
	if o == nil || IsNil(o.OriginalLanguageSummary.Get()) {
		var ret string
		return ret
	}
	return *o.OriginalLanguageSummary.Get()
}

// GetOriginalLanguageSummaryOk returns a tuple with the OriginalLanguageSummary field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetOriginalLanguageSummaryOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OriginalLanguageSummary.Get(), o.OriginalLanguageSummary.IsSet()
}

// HasOriginalLanguageSummary returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasOriginalLanguageSummary() bool {
	if o != nil && o.OriginalLanguageSummary.IsSet() {
		return true
	}

	return false
}

// SetOriginalLanguageSummary gets a reference to the given NullableString and assigns it to the OriginalLanguageSummary field.
func (o *SearchResponseDictItem1) SetOriginalLanguageSummary(v string) {
	o.OriginalLanguageSummary.Set(&v)
}
// SetOriginalLanguageSummaryNil sets the value for OriginalLanguageSummary to be an explicit nil
func (o *SearchResponseDictItem1) SetOriginalLanguageSummaryNil() {
	o.OriginalLanguageSummary.Set(nil)
}

// UnsetOriginalLanguageSummary ensures that no value is present for OriginalLanguageSummary, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetOriginalLanguageSummary() {
	o.OriginalLanguageSummary.Unset()
}

// GetImageDescription returns the ImageDescription field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetImageDescription() string {
	if o == nil || IsNil(o.ImageDescription.Get()) {
		var ret string
		return ret
	}
	return *o.ImageDescription.Get()
}

// GetImageDescriptionOk returns a tuple with the ImageDescription field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetImageDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ImageDescription.Get(), o.ImageDescription.IsSet()
}

// HasImageDescription returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasImageDescription() bool {
	if o != nil && o.ImageDescription.IsSet() {
		return true
	}

	return false
}

// SetImageDescription gets a reference to the given NullableString and assigns it to the ImageDescription field.
func (o *SearchResponseDictItem1) SetImageDescription(v string) {
	o.ImageDescription.Set(&v)
}
// SetImageDescriptionNil sets the value for ImageDescription to be an explicit nil
func (o *SearchResponseDictItem1) SetImageDescriptionNil() {
	o.ImageDescription.Set(nil)
}

// UnsetImageDescription ensures that no value is present for ImageDescription, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetImageDescription() {
	o.ImageDescription.Unset()
}

// GetCrawlDate returns the CrawlDate field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetCrawlDate() time.Time {
	if o == nil || IsNil(o.CrawlDate.Get()) {
		var ret time.Time
		return ret
	}
	return *o.CrawlDate.Get()
}

// GetCrawlDateOk returns a tuple with the CrawlDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetCrawlDateOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.CrawlDate.Get(), o.CrawlDate.IsSet()
}

// HasCrawlDate returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasCrawlDate() bool {
	if o != nil && o.CrawlDate.IsSet() {
		return true
	}

	return false
}

// SetCrawlDate gets a reference to the given NullableTime and assigns it to the CrawlDate field.
func (o *SearchResponseDictItem1) SetCrawlDate(v time.Time) {
	o.CrawlDate.Set(&v)
}
// SetCrawlDateNil sets the value for CrawlDate to be an explicit nil
func (o *SearchResponseDictItem1) SetCrawlDateNil() {
	o.CrawlDate.Set(nil)
}

// UnsetCrawlDate ensures that no value is present for CrawlDate, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetCrawlDate() {
	o.CrawlDate.Unset()
}

// GetContentType returns the ContentType field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *SearchResponseDictItem1) GetContentType() string {
	if o == nil || IsNil(o.ContentType.Get()) {
		var ret string
		return ret
	}
	return *o.ContentType.Get()
}

// GetContentTypeOk returns a tuple with the ContentType field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SearchResponseDictItem1) GetContentTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ContentType.Get(), o.ContentType.IsSet()
}

// HasContentType returns a boolean if a field has been set.
func (o *SearchResponseDictItem1) HasContentType() bool {
	if o != nil && o.ContentType.IsSet() {
		return true
	}

	return false
}

// SetContentType gets a reference to the given NullableString and assigns it to the ContentType field.
func (o *SearchResponseDictItem1) SetContentType(v string) {
	o.ContentType.Set(&v)
}
// SetContentTypeNil sets the value for ContentType to be an explicit nil
func (o *SearchResponseDictItem1) SetContentTypeNil() {
	o.ContentType.Set(nil)
}

// UnsetContentType ensures that no value is present for ContentType, not even an explicit nil
func (o *SearchResponseDictItem1) UnsetContentType() {
	o.ContentType.Unset()
}

// GetAsStringKey returns the AsStringKey field value
func (o *SearchResponseDictItem1) GetAsStringKey() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AsStringKey
}

// GetAsStringKeyOk returns a tuple with the AsStringKey field value
// and a boolean to check if the value has been set.
func (o *SearchResponseDictItem1) GetAsStringKeyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AsStringKey, true
}

// SetAsStringKey sets field value
func (o *SearchResponseDictItem1) SetAsStringKey(v string) {
	o.AsStringKey = v
}

func (o SearchResponseDictItem1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SearchResponseDictItem1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["article_url"] = o.ArticleUrl
	toSerialize["article_id"] = o.ArticleId
	toSerialize["classification"] = o.Classification
	toSerialize["country"] = o.Country
	toSerialize["source_id"] = o.SourceId
	toSerialize["page_rank"] = o.PageRank
	toSerialize["domain_url"] = o.DomainUrl
	toSerialize["eng_title"] = o.EngTitle
	toSerialize["entities"] = o.Entities
	if o.ImageUrl.IsSet() {
		toSerialize["image_url"] = o.ImageUrl.Get()
	}
	toSerialize["keywords"] = o.Keywords
	toSerialize["language"] = o.Language
	toSerialize["pub_date"] = o.PubDate
	toSerialize["summary"] = o.Summary
	if o.KeyPoints != nil {
		toSerialize["key_points"] = o.KeyPoints
	}
	toSerialize["title"] = o.Title
	toSerialize["sentiment"] = o.Sentiment
	toSerialize["centroid_distance"] = o.CentroidDistance
	toSerialize["cluster_probability"] = o.ClusterProbability
	if !IsNil(o.MarkdownCitation) {
		toSerialize["markdown_citation"] = o.MarkdownCitation
	}
	if !IsNil(o.Provocative) {
		toSerialize["provocative"] = o.Provocative
	}
	if !IsNil(o.ReportingVoice) {
		toSerialize["reporting_voice"] = o.ReportingVoice
	}
	if o.EntityRelationGraph.IsSet() {
		toSerialize["entity_relation_graph"] = o.EntityRelationGraph.Get()
	}
	if o.GeoCoordinates != nil {
		toSerialize["geo_coordinates"] = o.GeoCoordinates
	}
	if o.Continent.IsSet() {
		toSerialize["continent"] = o.Continent.Get()
	}
	if o.Assets.IsSet() {
		toSerialize["assets"] = o.Assets.Get()
	}
	if o.SocialEmbeds != nil {
		toSerialize["social_embeds"] = o.SocialEmbeds
	}
	if o.Bias.IsSet() {
		toSerialize["bias"] = o.Bias.Get()
	}
	if o.Authors != nil {
		toSerialize["authors"] = o.Authors
	}
	if o.FullText.IsSet() {
		toSerialize["full_text"] = o.FullText.Get()
	}
	if o.OriginalLanguageSummary.IsSet() {
		toSerialize["original_language_summary"] = o.OriginalLanguageSummary.Get()
	}
	if o.ImageDescription.IsSet() {
		toSerialize["image_description"] = o.ImageDescription.Get()
	}
	if o.CrawlDate.IsSet() {
		toSerialize["crawl_date"] = o.CrawlDate.Get()
	}
	if o.ContentType.IsSet() {
		toSerialize["content_type"] = o.ContentType.Get()
	}
	toSerialize["as_string_key"] = o.AsStringKey

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *SearchResponseDictItem1) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"article_url",
		"article_id",
		"classification",
		"country",
		"source_id",
		"page_rank",
		"domain_url",
		"eng_title",
		"entities",
		"keywords",
		"language",
		"pub_date",
		"summary",
		"title",
		"sentiment",
		"centroid_distance",
		"cluster_probability",
		"as_string_key",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSearchResponseDictItem1 := _SearchResponseDictItem1{}

	err = json.Unmarshal(data, &varSearchResponseDictItem1)

	if err != nil {
		return err
	}

	*o = SearchResponseDictItem1(varSearchResponseDictItem1)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "article_url")
		delete(additionalProperties, "article_id")
		delete(additionalProperties, "classification")
		delete(additionalProperties, "country")
		delete(additionalProperties, "source_id")
		delete(additionalProperties, "page_rank")
		delete(additionalProperties, "domain_url")
		delete(additionalProperties, "eng_title")
		delete(additionalProperties, "entities")
		delete(additionalProperties, "image_url")
		delete(additionalProperties, "keywords")
		delete(additionalProperties, "language")
		delete(additionalProperties, "pub_date")
		delete(additionalProperties, "summary")
		delete(additionalProperties, "key_points")
		delete(additionalProperties, "title")
		delete(additionalProperties, "sentiment")
		delete(additionalProperties, "centroid_distance")
		delete(additionalProperties, "cluster_probability")
		delete(additionalProperties, "markdown_citation")
		delete(additionalProperties, "provocative")
		delete(additionalProperties, "reporting_voice")
		delete(additionalProperties, "entity_relation_graph")
		delete(additionalProperties, "geo_coordinates")
		delete(additionalProperties, "continent")
		delete(additionalProperties, "assets")
		delete(additionalProperties, "social_embeds")
		delete(additionalProperties, "bias")
		delete(additionalProperties, "authors")
		delete(additionalProperties, "full_text")
		delete(additionalProperties, "original_language_summary")
		delete(additionalProperties, "image_description")
		delete(additionalProperties, "crawl_date")
		delete(additionalProperties, "content_type")
		delete(additionalProperties, "as_string_key")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableSearchResponseDictItem1 struct {
	value *SearchResponseDictItem1
	isSet bool
}

func (v NullableSearchResponseDictItem1) Get() *SearchResponseDictItem1 {
	return v.value
}

func (v *NullableSearchResponseDictItem1) Set(val *SearchResponseDictItem1) {
	v.value = val
	v.isSet = true
}

func (v NullableSearchResponseDictItem1) IsSet() bool {
	return v.isSet
}

func (v *NullableSearchResponseDictItem1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSearchResponseDictItem1(val *SearchResponseDictItem1) *NullableSearchResponseDictItem1 {
	return &NullableSearchResponseDictItem1{value: val, isSet: true}
}

func (v NullableSearchResponseDictItem1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSearchResponseDictItem1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


