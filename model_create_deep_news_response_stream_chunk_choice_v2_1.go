/*
AskNews API

AskNews API [![status](https://status.asknews.app/api/badge/2/status?style=for-the-badge)](https://status.asknews.app/status/prod)

API version: 0.24.66
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package asknews

import (
	"encoding/json"
	"fmt"
)

// checks if the CreateDeepNewsResponseStreamChunkChoiceV21 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CreateDeepNewsResponseStreamChunkChoiceV21{}

// CreateDeepNewsResponseStreamChunkChoiceV21 struct for CreateDeepNewsResponseStreamChunkChoiceV21
type CreateDeepNewsResponseStreamChunkChoiceV21 struct {
	Index int32 `json:"index"`
	Delta Delta `json:"delta"`
	FinishReason NullableString `json:"finish_reason,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _CreateDeepNewsResponseStreamChunkChoiceV21 CreateDeepNewsResponseStreamChunkChoiceV21

// NewCreateDeepNewsResponseStreamChunkChoiceV21 instantiates a new CreateDeepNewsResponseStreamChunkChoiceV21 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateDeepNewsResponseStreamChunkChoiceV21(index int32, delta Delta) *CreateDeepNewsResponseStreamChunkChoiceV21 {
	this := CreateDeepNewsResponseStreamChunkChoiceV21{}
	this.Index = index
	this.Delta = delta
	return &this
}

// NewCreateDeepNewsResponseStreamChunkChoiceV21WithDefaults instantiates a new CreateDeepNewsResponseStreamChunkChoiceV21 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateDeepNewsResponseStreamChunkChoiceV21WithDefaults() *CreateDeepNewsResponseStreamChunkChoiceV21 {
	this := CreateDeepNewsResponseStreamChunkChoiceV21{}
	return &this
}

// GetIndex returns the Index field value
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) GetIndex() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Index
}

// GetIndexOk returns a tuple with the Index field value
// and a boolean to check if the value has been set.
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) GetIndexOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Index, true
}

// SetIndex sets field value
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) SetIndex(v int32) {
	o.Index = v
}

// GetDelta returns the Delta field value
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) GetDelta() Delta {
	if o == nil {
		var ret Delta
		return ret
	}

	return o.Delta
}

// GetDeltaOk returns a tuple with the Delta field value
// and a boolean to check if the value has been set.
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) GetDeltaOk() (*Delta, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Delta, true
}

// SetDelta sets field value
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) SetDelta(v Delta) {
	o.Delta = v
}

// GetFinishReason returns the FinishReason field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) GetFinishReason() string {
	if o == nil || IsNil(o.FinishReason.Get()) {
		var ret string
		return ret
	}
	return *o.FinishReason.Get()
}

// GetFinishReasonOk returns a tuple with the FinishReason field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) GetFinishReasonOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FinishReason.Get(), o.FinishReason.IsSet()
}

// HasFinishReason returns a boolean if a field has been set.
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) HasFinishReason() bool {
	if o != nil && o.FinishReason.IsSet() {
		return true
	}

	return false
}

// SetFinishReason gets a reference to the given NullableString and assigns it to the FinishReason field.
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) SetFinishReason(v string) {
	o.FinishReason.Set(&v)
}
// SetFinishReasonNil sets the value for FinishReason to be an explicit nil
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) SetFinishReasonNil() {
	o.FinishReason.Set(nil)
}

// UnsetFinishReason ensures that no value is present for FinishReason, not even an explicit nil
func (o *CreateDeepNewsResponseStreamChunkChoiceV21) UnsetFinishReason() {
	o.FinishReason.Unset()
}

func (o CreateDeepNewsResponseStreamChunkChoiceV21) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CreateDeepNewsResponseStreamChunkChoiceV21) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["index"] = o.Index
	toSerialize["delta"] = o.Delta
	if o.FinishReason.IsSet() {
		toSerialize["finish_reason"] = o.FinishReason.Get()
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *CreateDeepNewsResponseStreamChunkChoiceV21) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"index",
		"delta",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCreateDeepNewsResponseStreamChunkChoiceV21 := _CreateDeepNewsResponseStreamChunkChoiceV21{}

	err = json.Unmarshal(data, &varCreateDeepNewsResponseStreamChunkChoiceV21)

	if err != nil {
		return err
	}

	*o = CreateDeepNewsResponseStreamChunkChoiceV21(varCreateDeepNewsResponseStreamChunkChoiceV21)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "index")
		delete(additionalProperties, "delta")
		delete(additionalProperties, "finish_reason")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableCreateDeepNewsResponseStreamChunkChoiceV21 struct {
	value *CreateDeepNewsResponseStreamChunkChoiceV21
	isSet bool
}

func (v NullableCreateDeepNewsResponseStreamChunkChoiceV21) Get() *CreateDeepNewsResponseStreamChunkChoiceV21 {
	return v.value
}

func (v *NullableCreateDeepNewsResponseStreamChunkChoiceV21) Set(val *CreateDeepNewsResponseStreamChunkChoiceV21) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateDeepNewsResponseStreamChunkChoiceV21) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateDeepNewsResponseStreamChunkChoiceV21) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateDeepNewsResponseStreamChunkChoiceV21(val *CreateDeepNewsResponseStreamChunkChoiceV21) *NullableCreateDeepNewsResponseStreamChunkChoiceV21 {
	return &NullableCreateDeepNewsResponseStreamChunkChoiceV21{value: val, isSet: true}
}

func (v NullableCreateDeepNewsResponseStreamChunkChoiceV21) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateDeepNewsResponseStreamChunkChoiceV21) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


